const state = {
    name: "", country: "", age: 17, personality: "balanced", day: 1, totalDays: 30,
...
const COUNTRIES = {
    "United Arab Emirates": { flag: "🇦🇪" }, Afghanistan: { flag: "🇦🇫" }, Pakistan: { flag: "🇵🇰" },
    India: { flag: "🇮🇳" }, Spain: { flag: "🇪🇸" },
};
const PERSONALITIES = {
    balanced: { label: "Pretty balanced" }, social: { label: "Outgoing" }, academic: { label: "Studious" },
    ambitious: { label: "Driven" }, calm: { label: "Easygoing" },
};
const PLACES = [
...
const HOURS = { home: [0, 1440], school: [450, 870], work: [540, 1080], gym: [360, 1320], park: [360, 1320], mall: [600, 1320], cafe: [420, 1320], hospital: [0, 1440] };
const JOBS = [
    { title: "Shop Assistant", education: 0, reputation: 0, pay: 55 }, { title: "Office Assistant", education: 35, reputation: 15, pay: 75 },
...
function startGame() {
    ...
    state.age = clamp(parseInt(document.getElementById("age").value, 10) || 17, 16, 80); state.personality = document.getElementById("personality").value;
    state.day = 1; state.hour = 7; state.minute = 30; state.cash = 100; state.debt = 0; state.groceries = { meals: 7, drinks: 10 }; state.location = "home"; state.ended = false;
    state.stats = { health: 100, energy: 100, hunger: 100, hydration: 100, hygiene: 100, mood: 80 }; state.skills = { education: 0, social: 0, fitness: 0, work: 0 }; state.feed = []; state.history = []; state.currentSection = "places";
    state.npcs = NAMES.map((name, i) => ({ name, emoji: EMOJIS[i], relationship: 20 + Math.floor(Math.random() * 40) }));
    document.getElementById("intro").classList.add("hidden"); document.getElementById("game").classList.remove("hidden"); addFeed("You're starting out in " + state.country + "."); addFeed("Day 1. You're at home, and there's plenty of time to figure things out."); updateLocation(); render();
}
function updateLocation() { const place = PLACES.find(item => item.id === state.location); if (place) { document.getElementById("locationTitle").textContent = place.emoji + " " + place.name; document.getElementById("locationDesc").textContent = place.desc; } }
function available(placeId) { if (placeId === "school" && state.day > 15) return false; if (placeId === "work" && state.day <= 15) return false; const [open, close] = HOURS[placeId] || [0, 1440]; const now = state.hour * 60 + state.minute; return (open === 0 && close === 1440) || (now >= open && now < close); }
function goTo(placeId) { if (state.ended) return; const place = PLACES.find(item => item.id === placeId); if (!place) return; if (state.location === placeId) addFeed("You're already at " + place.name + "."); else if (!available(placeId)) addFeed(place.name + " is closed right now.", "bad"); else { state.location = placeId; addMinutes(15); if (!state.ended) addFeed("You went to " + place.name + "."); updateLocation(); } render(); }
...
function phone() { showModal("📱 Phone", "<button class='option' onclick='bankApp()'><span class='opt-title'>🏦 Bank</span><span class='opt-sub'>Check balance and loans</span></button><button class='option' onclick='messagesApp()'><span class='opt-title'>💬 Messages</span><span class='opt-sub'>Talk to your friends</span></button><button class='option' onclick='jobsApp()'><span class='opt-title'>💼 Jobs</span><span class='opt-sub'>Check your work options</span></button>"); }
function bankApp() { let html = "<p>Balance: <strong>" + money(state.cash) + "</strong></p><p>Debt: <strong>" + money(state.debt) + " / " + money(state.maxDebt) + "</strong></p>"; if (state.debt < state.maxDebt) html += "<button class='option' onclick='takeLoan(50)'>Take AED 50 loan</button><button class='option' onclick='takeLoan(100)'>Take AED 100 loan</button>"; if (state.debt > 0) html += "<button class='option' onclick='repayLoan()'>Repay debt</button>"; showModal("🏦 Bank", html); }
function takeLoan(amount) { const actual = Math.min(amount, state.maxDebt - state.debt); if (actual <= 0) return; state.debt += actual; state.cash += actual; addFeed("You took a loan of " + money(actual) + ".", "bad"); closeModal(); checkGameOver(); render(); }
function repayLoan() { const amount = Math.min(state.cash, state.debt); state.cash -= amount; state.debt -= amount; addFeed("You repaid " + money(amount) + " of your debt.", "good"); closeModal(); render(); }
function messagesApp() { const html = state.npcs.map(npc => "<button class='option' onclick='chatWith(" + JSON.stringify(npc.name) + ")'><span class='opt-title'>" + npc.emoji + " " + npc.name + "</span><span class='opt-sub'>Relationship: " + npc.relationship + "</span></button>").join(""); showModal("💬 Messages", html || "<p>You have no contacts yet.</p>"); }
function chatWith(name) { showModal("💬 " + name, "<button class='option' onclick='chatAction(" + JSON.stringify(name) + ",5)'>😊 Friendly chat (+5)</button><button class='option' onclick='chatAction(" + JSON.stringify(name) + ",10)'>🎁 Send a gift (+10, AED 20)</button>"); }
function chatAction(name, amount) { if (amount === 10) { if (state.cash < 20) { addFeed("You can't afford a gift right now.", "bad"); return render(); } state.cash -= 20; } const npc = state.npcs.find(item => item.name === name); if (npc) npc.relationship = clamp(npc.relationship + amount, 0, 100); addFeed("You chatted with " + name + "."); closeModal(); render(); }
function jobsApp() { if (state.day <= 15) return showModal("💼 Jobs", "<p>Jobs open up after Day 15. For now, focus on school.</p>"); const job = getJob(); showModal("💼 Jobs", "<p>Your best option right now is <strong>" + job.title + "</strong> — AED " + job.pay + " per shift.</p><button class='option' onclick='closeModal();goTo(\"work\")'>Go to work</button>"); }
function historyModal() { const html = state.history.length ? state.history.slice().reverse().slice(0, 30).map(item => "<div class='feed-item " + item.type + "'><span class='time-tag'>Day " + item.day + " " + item.time + "</span>" + item.text + "</div>").join("") : "<p>No history yet.</p>"; showModal("📜 History", html); }
...
function renderMainGrid() { const grid = document.getElementById("mainGrid"); document.getElementById("sectionTitle").textContent = state.currentSection[0].toUpperCase() + state.currentSection.slice(1); if (state.currentSection === "places") grid.innerHTML = PLACES.filter(place => (place.id !== "school" || state.day <= 15) && (place.id !== "work" || state.day > 15)).map(place => "<div class='card' onclick='goTo(" + JSON.stringify(place.id) + ")'><div class='emoji'>" + place.emoji + "</div><div class='name'>" + place.name + "</div><div class='desc'>" + place.desc + "</div></div>").join(""); else if (state.currentSection === "activities") grid.innerHTML = (ACTIVITIES[state.location] || []).map(item => "<div class='card' onclick='doActivity(" + JSON.stringify(item.id) + ")'><div class='emoji'>" + item.emoji + "</div><div class='name'>" + item.name + "</div><div class='desc'>" + (item.cost ? money(item.cost) + " · " : "") + item.time + " min</div></div>").join(""); else grid.innerHTML = TRAVEL.map(([country, emoji, cost]) => "<div class='card' onclick='travelTo(" + JSON.stringify(country) + ")'><div class='emoji'>" + emoji + "</div><div class='name'>" + country + "</div><div class='desc'>" + money(cost) + "</div></div>").join(""); }

document.addEventListener("DOMContentLoaded", () => { const name = document.getElementById("name"); if (name) name.focus(); });
